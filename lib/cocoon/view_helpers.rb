module Cocoon
  module ViewHelpers
    ...
    # Function is very similar to the .link_to_add_association except that, at the end, it adds
    # a button instead of a link.

    def button_to_add_association(*args, &block)
      if block_given?
        button_to_add_association(capture(&block), *args)
      elsif args.first.respond_to?(:object)
        association = args.second
        name        = I18n.translate("cocoon.#{association}.add", default: I18n.translate('cocoon.defaults.add'))

        button_to_add_association(name, *args)
      else
        name, f, association, html_options = *args
        html_options ||= {}
